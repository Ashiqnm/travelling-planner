...
void addDestination(struct Destination destinations[], int* destCount) {
    if (*destCount < 10) {
        ...
        scanf("%99s", dest.name); // Limit input to avoid buffer overflow
        printf("Enter destination description: ");
        scanf(" %[^\n]", dest.description); // Read full line including spaces
...
void createItinerary(struct Itinerary** head, struct Destination destinations[], int destCount) {
    ...
    scanf("%99s", newItinerary->destination); // Limit input to avoid buffer overflow
    printf("Enter date for the itinerary: ");
    scanf("%19s", newItinerary->date); // Limit input to avoid buffer overflow
